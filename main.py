...
def detect_problem_type(df, target):
    ...
    # Treat anything non-numeric as classification
    if not pd.api.types.is_numeric_dtype(y):
        return "Classification"

    # Numeric but low unique values → classification
    if y.nunique() <= 10:
        return "Classification"
...
@st.cache_data
def preprocess_features(_df, features, config=None, _target_values=None):
    """
    Preprocess features with advanced handling for high-cardinality categoricals.
    _target_values: Series of target values (needed for target encoding)
    """
    if not features:
        return pd.DataFrame()
    ...
    if config is None:
        config = getattr(st.session_state, 'preprocessing_config', {
            ...
            'high_cardinality_method': 'Drop them (default)',
            'top_n_categories': 10,
            'enabled': True
        })
    
    valid_features = [f for f in features if f in _df.columns]
    if not valid_features:
        return pd.DataFrame()
    
    X = _df[valid_features].copy()
    
    # IF PREPROCESSING DISABLED
    if not config.get('enabled', True):
        st.warning("⚠️ Using raw features without preprocessing. Only numeric columns will be used.")
    ...
    if drop_cols:
        ...
        st.info(f"📉 Dropped {len(drop_cols)} columns due to >{config['missing_threshold']*100:.0f}% missing values")
    ...
    if cat_cols:
        ...
        # Now handle high cardinality
        high_card_method = config.get('high_cardinality_method', 'Drop them (default)')
        ...
        for col in cat_cols:
            ...
            if num_unique <= cardinality_threshold:
                ...
                high_card_cols.append((col, num_unique))
        
        # Handle low cardinality categoricals (simple label encoding)
        ...
        if high_card_cols:
            if "Drop them" in high_card_method:
                for col, num_unique in high_card_cols:
                    X = X.drop(columns=[col])
                    dropped_cols.append(f"{col} ({num_unique})")
                st.info(f"🗑️ Dropped {len(high_card_cols)} high-cardinality features: {', '.join(dropped_cols[:5])}{'...' if len(dropped_cols) > 5 else ''}")
            
            elif "Target Encoding" in high_card_method:
                if _target_values is not None:
                    ...
                    for col, num_unique in high_card_cols:
                        try:
                            ...
                            }).groupby('category')['target'].mean()
                            
                            # Map to original column
                            X[f'{col}_encoded'] = X[col].map(target_means)
                            X = X.drop(columns=[col])
                    ...
                    if encoded_cols:
                        st.success(f"✓ Target-encoded {len(encoded_cols)} high-cardinality features: {', '.join(encoded_cols[:5])}{'...' if len(encoded_cols) > 5 else ''}")
                else:
                    st.warning("⚠️ Target encoding requires target values. Dropping high-cardinality features instead.")
                    for col, num_unique in high_card_cols:
                        X = X.drop(columns=[col])
            
            elif "Frequency Encoding" in high_card_method:
                encoded_cols = []
                for col, num_unique in high_card_cols:
                    try:
                ...
                if encoded_cols:
                    st.success(f"✓ Frequency-encoded {len(encoded_cols)} high-cardinality features: {', '.join(encoded_cols[:5])}{'...' if len(encoded_cols) > 5 else ''}")
            
            elif "Keep top" in high_card_method:
                top_n = config.get('top_n_categories', 10)
                encoded_cols = []
                for col, num_unique in high_card_cols:
                    try:
                ...
                if encoded_cols:
                    st.success(f"✓ Kept top {top_n} categories for {len(encoded_cols)} features: {', '.join(encoded_cols[:5])}{'...' if len(encoded_cols) > 5 else ''}")
            
            elif "Hash Encoding" in high_card_method:
                encoded_cols = []
                n_features = 10  # Number of hash features
                for col, num_unique in high_card_cols:
                    try:
                        # Simple hash encoding
                        for i in range(n_features):
                            X[f'{col}_hash_{i}'] = X[col].apply(lambda x: hash(str(x) + str(i)) % 1000)
                        X = X.drop(columns=[col])
                        encoded_cols.append(col)
                    except:
                        X = X.drop(columns=[col])
                ...
                if encoded_cols:
                    st.success(f"✓ Hash-encoded {len(encoded_cols)} high-cardinality features into {n_features} features each")
...
if st.session_state.page=="landing":
        ...
        if st.session_state.dataset_loaded and st.session_state.df is not None:
            ...
            with st.expander("📊 Data Quality Report", expanded=False):
                ...
                for col in df.select_dtypes(include=['object']).columns:
                    ...
                    if unique_count > 50:
                        cat_cardinality.append({'Column': col, 'Unique Values': unique_count})
                
                if cat_cardinality:
                    st.dataframe(pd.DataFrame(cat_cardinality), use_container_width=True)
                    st.warning("⚠️ These will be dropped during preprocessing unless you adjust the cardinality threshold or disable preprocessing.")
                else:
                    st.success("✓ No high cardinality categorical features detected!")
            ...
            with st.expander("⚙️ Preprocessing Options", expanded=False):
                ...
                enable_preprocessing = st.checkbox(
                    ...
                    value=True,
                    help="Uncheck to skip preprocessing and use only raw numeric features"
                )
                ...
                if enable_preprocessing:
                    ...
                    with col2:
                        ...
                        cardinality_threshold = st.slider(
                            ...
                            10, 200, 50, 10,
                            help="Categorical columns with more unique values than this will be dropped (unless handled below)",
                            key="cardinality_thresh"
                        )
                    
                    # ADVANCED OPTIONS FOR HIGH CARDINALITY
                    st.write("---")
                    st.write("**Advanced: High Cardinality Handling**")
                    
                    high_cardinality_method = st.selectbox(
                        ...
                        help="Choose how to handle features with many unique values",
                        key="high_card_method"
                    )
                    
                    top_n_categories = None
                    if "Keep top" in high_cardinality_method:
                        top_n_categories = st.slider(
                            "Keep top N most frequent categories",
                            5, 50, 10, 5,
                            help="Other categories will be grouped as 'Other'",
                            key="top_n_cats"
                        )
                    ...
                    st.success(f"✓ Config saved: {high_cardinality_method}")
                else:
                    st.session_state.preprocessing_config = {
                        'enabled': False
                    }
                    st.warning("⚠️ Preprocessing disabled. Only numeric features will be used!")
            ...
            try:
                ...
                if target in features:
                    ...
                    if working_df.shape[0] < 10:
                        st.error("Not enough rows for training after cleaning target. Need at least 10 rows.")
                        st.stop()
                    ...
                    working_df,
                    features,
                    getattr(st.session_state, 'preprocessing_config', None),
                    _target_values=working_df[target]  # Pass target for target encoding
                    )

                    if X.empty:
                        st.error("No valid features remaining after preprocessing. Please select different features.")
                        st.stop()
                    ...
                    if problem_type == "Regression":
                        ALL_MODELS = [
                            "Linear Regression", "Ridge", "Lasso", "SVM", "KNN",
                    ...
                    preprocessing_enabled = st.session_state.preprocessing_config.get('enabled', True)
                    ...
                    if preprocessing_enabled:
                        preprocessing_info = [
                            ...
                            f"• Missing values: {st.session_state.preprocessing_config.get('num_impute', 'median')} imputation (numeric), {st.session_state.preprocessing_config.get('cat_impute', 'most_frequent')} (categorical)",
                            f"• High cardinality categorical features (>{st.session_state.preprocessing_config.get('cardinality_threshold', 50)} unique values): Dropped"
                        ]
                    else:
                        preprocessing_info = [
                            f"• Preprocessing: DISABLED - using only raw numeric features",
                            f"• Feature scaling: Applied to Linear Regression, SVM, KNN, MLP models",
                            f"• Target scaling: {'StandardScaler applied' if problem_type=='Regression' else 'Label encoding for categories'}"
                        ]
                    ...
                    if st.button("Train selected models", use_container_width=True):
                        if not selected_models:
                            ...
                            if use_sample and len(X) > 500:
                                sample_size = min(1000, int(len(X) * 0.3))
                                sampled_positions = np.random.RandomState(42).choice(
                                    len(X), size=sample_size, replace=False
                                )

                                # DON'T reprocess! Just subset
                                X = X.iloc[sampled_positions].reset_index(drop=True)
                                y_processed = y_processed[sampled_positions]
                            ...
                            if len(X) != len(y_processed):
                                st.error("Internal error: feature/target length mismatch after sampling. Aborting.")
                                st.stop()
    ...
    with tab4:
        if hasattr(st.session_state,'results') and st.session_state.results:
            ...
            summary_df[key_metric].sort_values().plot(kind='barh', ax=ax, color='skyblue')
            ax.set_xlabel(key_metric)
            ax.set_title(f'Model Comparison - {key_metric}')
            ax.axvline(x=summary_df[key_metric].mean(), color='r', linestyle='--', label='Average', linewidth=2)
            ax.legend()
            plt.tight_layout()
            st.pyplot(fig)
            plt.close()
                    ...
                    if i+j<num_models:
                        ...
                        with cols[j]:
                            ...
                            if model_name in ["Decision Tree", "Random Forest", "Gradient Boosting"]:
                                model_obj = st.session_state.models_trained.get(model_name)
                                if model_obj and hasattr(model_obj, 'feature_importances_'):
                                    try:
                                        feature_names = training_config.get('features_used', [])
            ...
            if key_metric in metrics:
                ...
                if problem_type=="Regression":
                    if best_score<0.6:
                        ...
                        recommendations.append("⚠️ **ENABLE PREPROCESSING** - You're missing important features!")
                    elif best_score<0.8:
...
- Features used: {len(training_config.get('features_used', []))}
